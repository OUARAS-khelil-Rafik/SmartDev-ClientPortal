...
const ConfirmDialog: React.FC<ConfirmDialogProps> = ({ open, title = 'Confirm', message = '', confirmLabel = 'Confirm', cancelLabel = 'Cancel', loading = false, onConfirm, onCancel, children }) => {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl max-w-md w-full p-6">
        ...
        {message && <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">{message}</p>}
        {children}
